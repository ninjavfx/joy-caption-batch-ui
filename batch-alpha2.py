...
PROMPT = "Write a short description of the image."
# PROMPT = "Write a short description of the image. Do NOT include information about people/characters that cannot be changed (like ethnicity, gender, age, etc). Include information about the subject expression Include information about lighting. Do NOT mention the backgound unless is relevant to the subject's pose If applicable, mention artificial or natural lighting sources."
...
parser.add_argument(
    ...
    type=str,
    default=None,
    help=f"Text file containing prompt to use (replaces default prompt). Plain text or JSON format supported.",
)
...
parser.add_argument(
    "--input-dir",
    type=str,
    default=str(SCRIPT_DIR / "input"),  # Use a direct default value
    help=f"Input directory for images. Default: 'input' subfolder of script dir",
)
parser.add_argument(
    "--output-dir",
    type=str,
...
def main():
    ...
    image_sources_provided = any([args.glob, args.filelist])
    ...
    image_paths = find_images(
        args.glob, args.filelist, use_default_input, args, input_folder
    )
    ...
    if not args.overwrite:
        ...
        image_paths = [
            ...
            if not (output_folder / Path(path).relative_to(input_folder))
            .with_suffix(".txt")
            .exists()
        ]
        logging.info(
    ...
    for batch in dataloader:
        ...
        for path, caption in zip(batch["paths"], captions):
            write_caption(Path(path), caption, args, input_folder, output_folder)
...
def write_caption(
    image_path: Path, caption: str, args, input_folder: Path, output_folder: Path
):
...
def parse_prompts(prompt_str: str | None, prompt_file: str | None) -> list[Prompt]:
    prompts = []
    global PROMPT  # Access the global PROMPT variable
    
    # First handle prompt from txt file - this has priority over --prompt arg
    if prompt_file:
        ...
        if not prompt_path.exists():
            ...
            try:
                ...
                if prompt_path.suffix.lower() in ['.json']:
                    ...
                    # Read the whole file as a single prompt (strip to remove trailing newlines)
                    txt_content = prompt_path.read_text(encoding='utf-8').strip()
                    if txt_content:
                        # Replace the global PROMPT with the text file content
                        PROMPT = txt_content
                        # Clear any existing prompts to ensure we only use the text file prompt
                        prompts = []
                        # Add the text content as a prompt with full weight
                        prompts.append(Prompt(prompt=txt_content, weight=1.0))
                        logging.info(f"Using prompt from text file: '{txt_content}'")
                    else:
                        logging.warning(f"Prompt file '{prompt_file}' is empty.")

            except json.JSONDecodeError as e:
                # If it's not valid JSON, treat it as a plain text file
                txt_content = prompt_path.read_text(encoding='utf-8').strip()
                if txt_content:
                    # Replace the global PROMPT with the text file content
                    PROMPT = txt_content
                ...
                logging.error(f"Failed to parse prompt file '{prompt_file}': {e}")
    
    # Then handle command line --prompt argument if no prompt file was used or if it failed
    elif prompt_str:
        prompts.append(Prompt(prompt=prompt_str, weight=1.0))

    # If no prompts added yet, use the global PROMPT as fallback
    ...
    # Log what prompt will be used
    if len(prompts) == 1:
        logging.info(f"Using prompt: '{prompts[0].prompt}'")
    else:
        logging.info(f"Using {len(prompts)} prompts with weights.")
...
def find_images(
    ...
    if glob:
        ...
        logging.info(
            f"Found {len(valid_glob_paths)} images using glob pattern '{glob}'."
        )
    ...
    if use_default_input:
        if not input_folder.exists():
            ...
            logging.error(f"Input path '{input_folder}' is not a directory.")
        else:
